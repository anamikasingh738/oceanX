...
abstract class Base {
	...
	public static function get_data( $params ) {
		...
		// Start Entry Query
		$query_cols = array( 'entry.id', 'entry.url', "DATE_FORMAT(captured, '%Y/%m/%d %H:%i:%S') as captured,form_id,form_plugin" );
		if ( $save_user_agent ) {
			$query_cols[] = 'entry.user_agent';
		}
		$entry_query = 'SELECT distinct ' . implode( ',', $query_cols ) . " FROM {$wpdb->prefix}fv_enteries as entry";
		// Add Joins
		$joins = array( 'INNER JOIN ' . $wpdb->prefix . 'fv_entry_meta as e1 ON (entry.id = e1.data_id )' );
		// phpcs:ignore
	...
	public function insert_enteries( $enteries ) {
		...
		$save_ua     = $db_settings['userAgent'];

		if ( 'yes' === $save_ua || $save_ua ) {
	...
	private function prepare_data_for_dashboard_widget( $params, $res ) {
		...
		if ( 'Last_7_Days' === $params['query_type'] || 'This_Week' === $params['query_type'] ) {
			$pre_from_date = gmdate( 'Y-m-d', strtotime( $params['fromDate'] . '-7 days' ) );
			$pre_to_date   = gmdate( 'Y-m-d', strtotime( $params['fromDate'] . '-1 days' ) );
		} elseif ( 'Last_30_Days' === $params['query_type'] ) {
			$pre_from_date = gmdate( 'Y-m-d', strtotime( $params['fromDate'] . '-30 days' ) );
			$pre_to_date   = gmdate( 'Y-m-d', strtotime( $params['fromDate'] . '-1 days' ) );
		} else {
			$pre_from_date = gmdate( 'Y-m-01', strtotime( 'first day of last month' ) );
			$pre_to_date   = gmdate( 'Y-m-t', strtotime( 'last day of last month' ) );
		}
		global $wpdb;
		...
		foreach ( $all_forms as $form_key => $form_value ) {
			if ( 'Caldera' === $form_value['plugin'] || 'caldera' === $form_value['plugin'] ) {
				...
				$dashboard_data['allFormsDataCount'][ $form_key ] = array(
		...
		$dashboard_widget_setting               = array();
	...
	public static function get_dates_from_range( $start, $end, $format = 'Y-m-d' ) {
		...
		for (
			...
			$store = gmdate( 'Y-m-d', $current_date );

			$array[ $store ] = (object) array(
				'Label'    => $store,
				'week'     => ( gmdate( 'z', $current_date ) + 1 ) . '(' . gmdate( 'y', $current_date ) . ')',
				'count'    => 0,
				'ordering' => gmdate( 'y', $current_date ) . '-' . ( gmdate( 'z', $current_date ) + 1 ),
			);
		}
		$array[] = new \stdClass();
		unset( $array[0] );
	...
	public static function get_month_range( $start_date, $end_date ) {
		...
		foreach ( $period as $dt ) {
			$months[ $dt->format( 'M' ) . '(' . $dt->format( 'y' ) . ')' ] = (object) array(
	...
	public static function get_date_range_ror_all_weeks( $start, $end ) {
		...
		while ( $fweek['sunday'] < $lweek['sunday'] ) {
			$week_dates[ $fweek['monday'] ] = (object) array(
				'Label'    => $fweek['monday'],
				'week'     => '',
				'count'    => 0,
				'ordering' => '',
		...
		$week_dates[ $lweek['monday'] ] = (object) array(
			'Label'    => $lweek['monday'],
			'week'     => '',
			'count'    => 0,
			'ordering' => '',
	...
	public static function get_date_range_for_week( $date ) {
		...
		if ( 'Monday' === $date_time->format( 'l' ) ) {
			$monday = gmdate( 'Y-m-d', strtotime( $date ) );
		} else {
			$monday = gmdate( 'Y-m-d', strtotime( 'last monday', strtotime( $date ) ) );
		}

		$sunday = 'Sunday' === $date_time->format( 'l' ) ? gmdate( 'Y-m-d', strtotime( $date ) ) : gmdate( 'Y-m-d', strtotime( 'next sunday', strtotime( $date ) ) );
	...
	public static function is_save_ip_user_agent( $settings ) {
		$save_user_agent = '';
		if ( false === $settings ) {
			$save_ip = get_option( 'fv-ip-save' );
			if ( false === $save_ip ) {
				$save_ip = true;
			}
		} else {
			$save_ip         = $settings['ip'];
			$save_user_agent = $settings['userAgent'];
		}

		return array( $save_ip, $save_user_agent );
	}
}
