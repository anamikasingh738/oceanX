...
class Cf7 extends Base {
	...
	public function __construct() {
		...
		add_filter( 'wpcf7_mail_components', array( $this, 'update_mail_content' ), 10, 3 );
	}
	...
	protected function set_skip_fields() {
		...
		$this->skip_fields = array( 'g-recaptcha-response', '_wpcf7', '_wpcf7_version', '_wpcf7_locale', '_wpcf7_unit_tag', '_wpcf7_container_post' );
	}
	// phpcs:ignore
	...
	public static function get_submission_data( $param ) {
		...
		$filter_col_id_res = $wpdb->get_results( $wpdb->prepare( '%s', $filter_col_id ), ARRAY_A );
		$entry_id          = array();
		...
		$entry_res = $wpdb->get_results( $wpdb->prepare( '%s', $entry_query ), ARRAY_A );

		$meta_data  = array();
		$ip_checker = '';
		if ( 'yes' === $gdpr_settings['ip'] ) {
			$ip_checker = '';
		} else {
			$ip_checker = 'IP';
		}
