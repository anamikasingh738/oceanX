...
class Caldera extends Base {
	...
	public function __construct() {
		...
		add_filter( 'formvibes/forms', array( $this, 'forms' ) );
		// phpcs:ignore
	...
	public static function get_data( $params ) {
		...
		global $wpdb;
		$query_cols  = array( "DATE_FORMAT(ADDTIME(datestamp,'" . $time_zone . "' ), '%Y/%m/%d %H:%i:%S') as datestamp", 'entry.id', 'entry.form_id', 'entry.user_id', 'entry.status' );
		$entry_query = 'SELECT distinct ' . implode( ',', $query_cols ) . " FROM {$wpdb->prefix}cf_form_entries as entry ";

		// Add Joins
		$joins = array( "INNER JOIN {$wpdb->prefix}cf_form_entry_values as e1 ON (entry.id = e1.entry_id )" );
		// phpcs:ignore
	...
	public static function get_forms( $param = array() ) {
		// phpcs:ignore
		$post_type = $param;
